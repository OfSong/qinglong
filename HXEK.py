...
class RUN:
    ...
    def sendMsg(self):
    if self.send_UID:
        push_res = self.pushplus_send(one_msg)  # 调用 pushplus_send 方法
        print(push_res)
...
def pushplus_send(self, content):
    # 这里假设你已经设置了 PushPlus 的 token
    ...
    data = {
        ...
        "title": "鸿星尔克签到通知",  # 可自定义消息标题
        "content": content,  # 发送的消息内容
        "template": "txt"  # 消息类型，默认为文本
    }
    try:
        response = requests.post(url, json=data, headers=headers)
        if response.status_code == 200:
            return f"PushPlus推送成功：{response.json()}"
        else:
            return f"PushPlus推送失败，状态码：{response.status_code}"
    except Exception as e:
