...
ck = "2e9ded7b881d89f9451ac8913e95d8d2"  # 本地环境ck，环境变量存在此处不生效
...
class PGSH:
    def __init__(self, cki):
        ...
        self.accountCertificate_url = 'https://userapi.qiekj.com/user/accountCertificate'
        self.daily_quiz_url = "https://userapi.qiekj.com/integralRecord/statisticsAnswer"
    ...
    def daily_quiz(self):
        ...
        multipart_data = MultipartEncoder(
            fields={
                'number': str(current_number),  # 使用计算得到的数值
                'isCorrect': 'true',
                'token': self.token
            }
        )
        ...
        # 生成并添加签名和时间戳
        sign, _, timestamp = self.sg(self.daily_quiz_url)
        headers['sign'] = sign
        headers['timestamp'] = str(timestamp)

        try:
            # 发送POST请求
            response = requests.post(self.daily_quiz_url, data=multipart_data, headers=headers,
                                     proxies=global_proxy if dl else None,
            ...
            # 检查请求是否成功
            if result['code'] == 0:
                if result['data'].get('isReward', False):
                    reward = result['data'].get('rewardNum', 0)
                    print(f"[{self.phone}] ✅每日答题成功，获得奖励：{reward}")
                else:
                    print(f"[{self.phone}] ✅每日答题已完成，今日无法再获得奖励")
            else:
                print(f"[{self.phone}] ❎每日答题失败：{result.get('msg', '未知错误')}")

        except requests.exceptions.RequestException as e:
            print(f"[{self.phone}] ❎每日答题出错：网络请求失败，错误信息：{str(e)}")
        except ValueError:
            print(f"[{self.phone}] ❎每日答题出错：无法解析响应JSON")
        except Exception as e:
            print(f"[{self.phone}] ❎每日答题出错：{str(e)}")

    def check_and_buchong_info(self):
        check_url = "https://userapi.qiekj.com/user/info"
        buchong_url = self.accountCertificate_url
        data = {'token': self.token}
        headers = self.hd.copy()

        try:
            # 检查是否需要补充信息
            sign, _, timestamp = self.sg(check_url)
            headers['sign'] = sign
            ...
            if check_result.get('data', {}).get('isAuth') == 1:
                print(f"[{self.phone}] ✅补充信息已完成 ==> True")
                return

            # 需要补充信息，执行补充
            multipart_data = {
                'gender': str(random.choice([1, 2])),
                'vocation': str(random.randint(1, 10)),
                'token': self.token
            }

            sign, _, timestamp = self.sg(buchong_url)
            headers['sign'] = sign
            headers['timestamp'] = timestamp

            buchong_response = requests.post(buchong_url, data=multipart_data, headers=headers,
                                             proxies=global_proxy if dl else None,
                                             timeout=10, verify=False)
            result = buchong_response.json()

            if result['code'] == 0:
                print(f"[{self.phone}] ✅补充信息成功 ==> True")
                print(f"[{self.phone}] ℹ️ 提示: 信息已更新，无需重复提交")
            else:
                print(f"[{self.phone}] ❎补充信息失败: {result['msg']}")
                if "手速太快" in result['msg']:
                    print(f"[{self.phone}] 请求过于频繁，稍后将自动重试")
                    time.sleep(5)  # 简单的延迟，可以根据需要调整

        except requests.RequestException as e:
            print(f"[{self.phone}] 网络请求异常: {str(e)}")
        except Exception as e:
            print(f"[{self.phone}] ❎补充信息出错: {str(e)}")
...
if __name__ == '__main__':
    print(f"当前版本: {v}")
    print("QQ交流群：666666666  修改:853561212")
